...
async function runBackfill() {
  ...
  try {
    ...
    await trx('dmr_lab_report').del();
    await trx('dmr_moisture').del();
    await trx('dmr_attendance').del();
    await trx('dmr_wheat_locations').del();
    await trx('dmr_jute_bags').del();
    await trx('dmr_salesman_sales').del();
    await trx('dmr_todays_production').del();
    await trx('dmr_sales_pending').del();
    await trx('dmr_sales_report').del();
    await trx('dmr_finish_stock').del();
    // Clear parent table
    await trx('daily_mill_reports').del();

    console.log('Fetching master data (products and salesmen)...');
    const products = await trx('master_products').select('*');
    const salesmen = await trx('master_salesmen').select('*');

    if (products.length === 0) {
      throw new Error('No master products found. Please run master data seed first.');
    }
    ...
    for (const dateStr of dates) {
      ...
      const [reportId] = await trx('daily_mill_reports').insert({
        report_date: dateStr,
      ...
      await trx('dmr_finish_stock').insert(finishStockRows);
      ...
      await trx('dmr_sales_report').insert(salesReportRows);
      ...
      await trx('dmr_sales_pending').insert(salesPendingRows);
      ...
      await trx('dmr_todays_production').insert(todaysProductionRows);
      ...
      if (salesmen.length > 0) {
        ...
        await trx('dmr_salesman_sales').insert(salesmanSalesRows);
      }
      ...
      await trx('dmr_attendance').insert(attendanceRows);
      ...
      await trx('dmr_moisture').insert(moistureRows);
    }
